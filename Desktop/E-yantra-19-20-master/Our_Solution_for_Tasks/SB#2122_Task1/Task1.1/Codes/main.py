...
def process(ip_image):
    ...
    cx=[];cy=[];
    for (l,u) in list1 :
          m1=cv2.inRange(img1,np.array(l),np.array(u));
          x=cv2.bitwise_and(img1,img1,mask=m1);
          b=cv2.cvtColor(x,cv2.COLOR_BGR2GRAY)
          _, c= cv2.threshold(b, 0, 255, cv2.THRESH_BINARY)
          s2=cv2.moments(c)
          cx.append(s2['m10']/s2['m00'])
          cy.append(s2['m01']/s2['m00'])   
          
   
    ang = math.degrees(math.atan2(cy[0]-x1, cx[0]-x1) - math.atan2(cy[1]-y1, cx[1]-y1))
    if ang<0 :
       ang=ang*-1;
    ang1=360-ang
    ang2=min(ang,ang1)
